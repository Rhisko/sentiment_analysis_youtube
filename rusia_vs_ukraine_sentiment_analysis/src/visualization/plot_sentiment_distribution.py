...
def create_grafik_sentiment_distribution(file_path):
    ...
    pivot_table = df.pivot_table(
        index="comment_target", 
        columns="sentiment", 
        aggfunc="size", 
        fill_value=0
    )
    
    # Calculate total comments per comment_target
    ...
    plt.title("Distribusi Sentimen Berdasarkan comment_target")
    plt.xlabel("comment_target")
    ...
    # Pie charts for each comment_target
    for comment_target in pivot_table.index:
        sentiment_counts = pivot_table.drop(columns="total_comments").loc[comment_target]
        total_comments = pivot_table.loc[comment_target, "total_comments"]
        ...
        plt.title(f"Distribusi Sentimen untuk comment_target {comment_target}\nTotal Komentar: {total_comments}")
